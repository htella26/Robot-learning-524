...
class ColorSortingEnv(ManipulationEnv):
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            @sensor(modality=modality)
            def gripper_to_cube_distance(obs_cache):
                gripper_pos = np.array(self.sim.data.site_xpos[self.robots[0].eef_site_id])  # FIXED
                cube_pos = np.array(self.sim.data.body_xpos[self.cube_body_id])
                return np.linalg.norm(gripper_pos - cube_pos)  # Euclidean distance

            sensors = [cube_pos, cube_quat, gripper_to_cube_distance]
